...
function migration_recuperer_lapage(
	$url,
	$trans = false,
	$get = 'GET',
	$taille_max = 1048576,
	...
	list($f, $fopen) = migration_init_http($get, $url, $refuser_gz, $uri_referer, $datas, _INC_DISTANT_VERSION_HTTP,
		$date_verif);
	if (!$f) {
...
function migration_init_http(
	...
	$f = migration_lance_requete($method, $scheme, $user, $host, $path, $port, $noproxy, $refuse_gz, $referer, $datas,
		$vers, $date);
	if (!$f) {
		// fallback : fopen
...
function migration_lance_requete(
	$method,
	$scheme,
	$user,
	$host,
